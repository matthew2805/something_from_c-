...
namespace TrySmthing
{
    class Program
    {
        ...
        public static List<string> ReadFromFile(string path)//that place
        {
            ...
            {
                str = str.Trim();
                if (!str.Equals("Plate Numbers") && !str.Equals("") && )//""-обработка последней идет n/!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!вот тут что-то добавить !!!!!!!!!!!!!!
                {
                    text.Add(str);
                }
            }
        ...
        static bool CheckElements(string[] numberPlate,Dictionary<string,string> garbage)//проверяем 2 ли слова в строке
        {
            if (numberPlate.Length != 2)
            {
                garbage.Add("Invalid", string.Join(" ",numberPlate)); ///я добавил вот эьто  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                return false;
            }
            else return true;
        ...
        static void Main(string[] args)
        {
            ...
            Dictionary<string, string> garbage = new Dictionary<string, string>();
            foreach (string number in text)//number-just strings with an exception of 1
            {
                string[] numberPlate = number.Split(" ");
                if (CheckElements(numberPlate,garbage) && CheckNumber(numberPlate[1]) && CheckString(numberPlate[0]))
                {
